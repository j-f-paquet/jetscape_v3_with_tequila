...
void Tequila::DoEnergyLoss(double deltaT, double Time, double Q2,
  ...
  bool active;         // wheter this parton is active in TEQUILA
  ...
  for (int i = 0; i < pIn.size(); i++) {
    ...
    userInfo = pIn[i].has_user_info<TEQUILAUserInfo>();
    // set user information to every parton
    if (!userInfo)
      pIn[i].set_user_info(new TEQUILAUserInfo());

    coherent = pIn[i].user_info<TEQUILAUserInfo>().coherent();
    sibling = pIn[i].user_info<TEQUILAUserInfo>().coherent_with();
    pAtSplit = pIn[i].user_info<TEQUILAUserInfo>().p_at_split();
    ...
    if (process == 0 || (coherent && (process < 5))) {
      ...
      if (coherent) {
        pOut[pOut.size() - 1].set_user_info(
            new TEQUILAUserInfo(coherent, sibling, pAtSplit));
      } else {
        pOut[pOut.size() - 1].set_user_info(new TEQUILAUserInfo());
      }
    ...
    if (std::abs(Id) == 1 || std::abs(Id) == 2 || std::abs(Id) == 3) {
      ...
      if (process == 1) {
        ...
        //  pOut[0].set_user_info(new TEQUILAUserInfo(coherentNew, pLabelNew, pAtSplitNew));
        //  pOut[1].set_user_info(new TEQUILAUserInfo(coherentNew, pLabel, pAtSplitNew));
        //}
...
int Tequila::DetermineProcess(double pRest, double T, double deltaTRest,
                              int Id) {
...
bool Tequila::isCoherent(Parton &pIn, int sibling, double T) {
  ...
  for (eIt = shower->edges_begin(), eEnd = shower->edges_end(); eIt != eEnd;
    ...
    if (eIt->target().outdeg() < 1) {
      ...
      if (p->plabel() == sibling) {
        ...
        if (p->has_user_info<TEQUILAUserInfo>()) {
          bool coherentSibling = p->user_info<TEQUILAUserInfo>().coherent();
          if (!coherentSibling)
            return coherentSibling;
        } else {
          JSWARN << "TEQUILAUserInfo not set!!"
                 << " plabel:" << p->plabel()
        ...
        FourVector pInitVec = pIn.user_info<TEQUILAUserInfo>().p_at_split();
...
RateElastic Tequila::getRateElasTotal(double pRest, double T) {
...
RateElastic Tequila::getRateElasPos(double u, double T) {
  RateElastic rate;
...
RateElastic Tequila::getRateElasNeg(double u, double T) {
  RateElastic rate;
...
RateConversion Tequila::getRateConv(double pRest, double T) {
  RateConversion rate;
...
double Tequila::getEnergyTransfer(double pRest, double T, int process) {
  double u = pRest / T; // making arguments in log to be dimensionless
...
double Tequila::getMomentumTransfer(double pRest, double omega, double T,
                                    int process) {
  double u = pRest / T; // making arguments in log to be dimensionless
  omega /= T;
...
// calculates the area under the envelope function when using the rejection method
// integrals had been solved analytically before
double Tequila::areaOmega(double u, int posNegSwitch, int process) {
...
double Tequila::areaQ(double u, double omega, int process) {
  double A, B;
  double areaQ;
...
FourVector Tequila::getNewMomentumElas(FourVector pVec, double omega,
                                       double q) {
...
void Tequila::readElasticRateOmega() {
  ifstream fin;
  string filename[2];
...
double Tequila::getRate_qqgamma(double p, double k) {
  return use_table(p, k, Gam.qqgamma, 3);
}

double Tequila::use_table(double p, double k, double dGamma[NP][NK],
                          int which_kind)
...
double Tequila::getElasticRateOmega(double u, double omega, int process) {
...
double Tequila::getElasticRateQ(double u, double omega, double q, int process) {
  if (q > std::abs(omega) &&
...
double Tequila::use_elastic_table_omega(double omega, int which_kind)
/* Uses the lookup table and simple interpolation to get the value
...
double Tequila::use_elastic_table_q(double omega, double q, int which_kind)
/* Uses the lookup table and simple interpolation to get the value
